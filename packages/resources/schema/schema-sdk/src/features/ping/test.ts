import { createSchemaSdk, getPing, parsePingResponse } from './index';
...
describe('getPing', () => {
  it('should call the ping endpoint and parse the payload', async () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      'https://api.example.com/ping?source=sdk',
      {
        method: 'GET',
        headers: undefined,
        signal: undefined,
  ...
  it('should throw when the endpoint returns a non-success status', async () => {
    ...
    await expect(
      getPing({ baseUrl: 'https://api.example.com', fetchFn }),
    ).rejects.toThrow(
...
describe('createSchemaSdk', () => {
  it('should call ping with the configured base URL', async () => {
    ...
    const sdk = createSchemaSdk({
      baseUrl: 'https://api.example.com',
      fetchFn,
    });

    await expect(
      sdk.ping({ query: { source: 'initialized-sdk' } }),
    ).resolves.toEqual({
      status: 'ok',
      checkedAt: '2026-01-01T00:00:00.000Z',
      uptimeSeconds: 42,
